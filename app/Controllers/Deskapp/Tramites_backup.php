...
class Tramites extends BaseController
{
    ...
    public function tramite()
    {
        try {
            ...
            $tramite_crud->fieldType('folio','hidden');
            $tramite_crud->fieldType('user_id','hidden');
            $tramite_crud->fieldType('created_at','hidden');
            $tramite_crud->fieldType('updated_at','hidden');
            $tramite_crud->fieldType('cobro_status_id','hidden');
            ...
            $tramite_crud->callbackEditForm(function ($data) use ($self, $tramite_crud){
            ...
            $tramite_crud->callbackBeforeInsert(function ($stateParameters) {
                $stateParameters->data['created_at'] = date('Y-m-d H:i:s');
                $stateParameters->data['updated_at'] = date('Y-m-d H:i:s');
                return $stateParameters;
            });

            $tramite_crud->callbackBeforeUpdate(function ($stateParameters) {
                $stateParameters->data['updated_at'] = date('Y-m-d H:i:s');
                return $stateParameters;
            });
            $tramiteAfterInsert = new TramiteAfterInsert($this->_getDbData());
            $tramite_crud->callbackAfterInsert(function ($stateParameters) use ($self, $tramiteAfterInsert){
            ...
            $tramite_crud->callbackAfterUpdate(function ($stateParameters) use ($self){
                $db = Database::connect();
                $db2 = $this->_getDbData();
                $session = session();
            ...
            // $tramite_salida->form1 = $tramite_salida->output;
            ...
            // $this->load->view('/deskapp/extra-pages/grocery_simple_page', $data);
            echo $this->_example_output($salida_total);
    ...
    public function mios()
    {
        try {
            ...
            $crud->setSubject('tramite', 'Mis Tramites');
            $crud->where([
            ...
            return $this->_simple_output($salida2);
        } catch (\Exception $e) {
            exit($e->getMessage());
        }
    }
