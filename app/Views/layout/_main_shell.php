<?php
$layoutVariant = $layoutVariant ?? 'legacy';
$isSglLayout = ($layoutVariant === 'sgl');
$assets = base_url('/public/assets');
$bodyClasses = trim(($bodyClass ?? '') . ($isSglLayout ? ' sgl-theme-2026' : ''));
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body<?= $bodyClasses !== '' ? ' class="' . esc($bodyClasses, 'attr') . '"' : '' ?><?= $isSglLayout ? ' data-layout-theme="sgl-2026"' : '' ?>>
        <?php if ($isSglLayout): ?>
        ...
        <script>
            (function (window, document, $) {
                ...
                $(function () {
                    ...
                    $(document).on('focus', 'select.form-control, select.custom-select, select.form-select', function () {
                        if (shouldEnhanceSelect(this)) {
                            enhanceSelects(this);
                        }
                    });
