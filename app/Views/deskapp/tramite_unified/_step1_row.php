 ...
 *
 * When isApprovedLock is true the rail renders clean read-only text instead
 * of the form with disabled inputs.
 */

$canEdit          = !empty($prototypeStep1Form['canEdit']);
$isApprovedLock   = !empty($prototypeStep1Form['isApprovedLock']);
...
$svcPrincipalLabel = (string) ($values['tipo_principal_label'] ?? 'Sin tipo principal');
$svcAsociados = [];
foreach ($svcServices as $svcRow) {
    ...
    if (!empty($svcRow['is_principal']) || $svcRowTipoId === $svcPrincipalId) {
        ...
        $svcAsociados[] = ['asociado_id' => (int) ($svcRow['asociado_id'] ?? 0), 'tra_tipos_id' => $svcRowTipoId, 'label' => $svcRowLabel];
    }
}

// Read-only resolved labels
$roCliente   = (string) ($values['cliente_name'] ?? '');
$roEjecutivo = (string) ($values['ejecutivo_name'] ?? '');
$roEntidad   = (string) ($values['entidad_name'] ?? '');
?>
<section class="tul-step-row tul-step-row--1" data-step-row="1">
    <header class="tul-step-row__header">
        ...
        <?php if ($isApprovedLock): ?>
            <span class="tul-readonly-badge tul-readonly-badge--approved">
                <i class="icon-check-circle"></i> Aprobado
            </span>
        <?php elseif (!$canEdit): ?>
            <span class="tul-readonly-badge"><i class="icon-lock"></i> Solo lectura</span>
        <?php endif; ?>
    </header>

    <div class="tul-three-rail">
        <!-- Carril izquierdo: datos base -->
        <div class="tul-rail tul-rail--form" data-rail="form">
            <?php if ($isApprovedLock): ?>
                <!-- READ-ONLY MODE: clean label + value display, no form -->
                <div class="tul-ro-card">
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Folio</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['folio'] ?? '—')) ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Cliente</span>
                        <span class="tul-ro-card__value"><?= esc($roCliente ?: '—') ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Ejecutivo</span>
                        <span class="tul-ro-card__value"><?= esc($roEjecutivo ?: '—') ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Contrato</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['contrato'] ?? '—')) ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Unidad</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['unidad'] ?? '—')) ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Serie</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['serie'] ?? '—')) ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Placas</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['placas'] ?? '—')) ?></span>
                    </div>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Entidad</span>
                        <span class="tul-ro-card__value"><?= esc($roEntidad ?: '—') ?></span>
                    </div>
                    <?php if ((string) ($values['observaciones'] ?? '') !== ''): ?>
                    <div class="tul-ro-card__row tul-ro-card__row--wide">
                        <span class="tul-ro-card__label">Observaciones</span>
                        <span class="tul-ro-card__value"><?= esc((string) ($values['observaciones'] ?? '')) ?></span>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="tul-ro-card" style="margin-top:12px">
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Tipo principal</span>
                        <span class="tul-ro-card__value"><?= esc($svcPrincipalLabel) ?></span>
                    </div>
                    <?php foreach ($svcAsociados as $assoc): ?>
                    <div class="tul-ro-card__row">
                        <span class="tul-ro-card__label">Asociado</span>
                        <span class="tul-ro-card__value"><?= esc($assoc['label']) ?></span>
                    </div>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="tul-rail tul-rail--docs" data-rail="docs">
            <?php if ($docsCanView): ?>
                <?php if ($docsCanUpload && !$isApprovedLock): ?>
                ...
                <?php elseif (!$docsCanUpload && $docsBlockedReason !== '' && !$isApprovedLock): ?>
                ...
                <div class="tul-gallery" data-tul-gallery data-tul-step="1">
                    ...
                    $uploadedDocuments = array_filter($documents ?? [], function($d) {
                        if (!empty($d['files']) && is_array($d['files'])) return count($d['files']) > 0;
                    ...
                    <?php if (!empty($uploadedDocuments)): ?>
                            ...
                            <div class="tul-gallery__item" data-tul-doc data-tul-doc-id="<?= $docId ?>">
                                <div class="tul-gallery__item-info">
                                    ...
                                    <?php foreach ($docFiles as $docFile): ?>
                                        <?php $singleUrl = (string) ($docFile['url'] ?? ''); $singleName = (string) ($docFile['name'] ?? ''); ?>
                                ...
                                <?php if ($docsCanDelete && !$isApprovedLock && $hasFile && $docId > 0 && $fileName !== ''): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="tul-gallery__empty">Aún no se han subido documentos.</p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
